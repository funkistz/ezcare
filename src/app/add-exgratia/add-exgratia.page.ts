...
@Component({
  selector: 'app-add-exgratia',
  templateUrl: './add-exgratia.page.html',
  styleUrls: ['./add-exgratia.page.scss'],
})
export class AddExgratiaPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.ionicForm = this.formBuilder.group({
      ...
      warranty_plan: ['', []],
      total_last_year_production: ['', []],
      total_this_year_production: ['', []],
  ...
  getInspection(id) {
    ...
    return this.firestore.collection('exgratia').doc(id).valueChanges().subscribe((data: any) => {
      ...
      if (data) {
        ...
        this.ionicForm.controls['total_last_year_production'].setValue(data.total_last_year_production);
        this.ionicForm.controls['total_this_year_production'].setValue(data.total_this_year_production);
        this.ionicForm.controls['total_last_year_claim'].setValue(data.total_last_year_claim);
        this.ionicForm.controls['total_this_year_claim'].setValue(data.total_this_year_claim);
  ...
  async addInspection() {
    ...
    this.firestore.collection('/exgratia/').add(data).then(() => {
      console.log('success');
      this.loading = false;
      this.loadingText = '';
      this.helper.presentToast('Ex Gratia successfully added.');
      this.navCtrl.back();
  ...
  updateInspectionStatus(status) {
    ...
    this.firestore.doc<any>('exgratia/' + this.inspection_id).update({
      status_changed_by: {
      ...
      this.loading = false;
      this.loadingText = '';
      this.helper.presentToast('Ex Gratia successfully updated.');
      this.navCtrl.back();
  ...
  imagePreview(src) {
    ...
    this.photoViewer.show(src, 'Ex Gratia', options);
    ...
    const alert = await this.alertController.create({
      ...
      buttons: [{
        ...
        handler: () => {
          ...
          this.firestore.doc<any>('exgratia/' + this.inspection_id).delete().then(() => {
            console.log('success');
            this.loading = false;
            this.loadingText = '';
            this.helper.presentToast('Ex Gratia successfully deleted.');
            this.navCtrl.back();
