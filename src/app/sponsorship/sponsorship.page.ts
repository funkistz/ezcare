...
export class SponsorshipPage implements OnInit {
  ...
  getTasks() {
    ...
    return this.firestore.collection('sponsorship', ref => ref.orderBy('date', 'desc')).snapshotChanges().subscribe((res) => {
      ...
      console.log('sponsorship', this.inspectionsTemp);
  ...
  addEndorsement() {
    this.router.navigate(['/staff-tabs/sponsorship/add']);
  }
  ...
  viewLog(inspection_id) {
    ...
    this.router.navigate(['/staff-tabs/sponsorship/add'], navigationExtras);
  }
